import { generateAndPostStartingPool, rollStartingPool } from "./fin/pools.ts";
  ...
  for (const rarity of rarityOrder) {
    ...
    if (cardsOfRarity.length > 0) {
      ...
      for (const card of cardsOfRarity) {
        const colorStr = card.colors.length > 0
          ? ` [${card.colors.join("")}]`
          : " [C]";
...
function printHistogram(histogram: ColorHistogram) {
  ...
  for (const [colorCode, data] of Object.entries(histogram)) {
    ...
    for (let i = 0; i < buckets; i++) {
      ...
      console.log(
        `    ${rangeStart.toString().padStart(2)}-${
          rangeEnd.toString().padEnd(2)
        }: ${bar} ${count}`,
      );
    }
    console.log();
...
async function testRollStartingPool() {
  console.log(
    `Testing rollStartingPool function with ${POOL_COUNT} pools...\n`,
  );
  ...
  try {
    ...
    const commonPools = pools.map((pool) =>
    ...
    let failedPools = 0;
    let colorFailures: { [color: string]: number } = {
      W: 0,
      U: 0,
      B: 0,
      R: 0,
    ...
    console.log(
      `Pools meeting color requirements: ${
        pools.length - failedPools
      }/${pools.length} (${
        ((pools.length - failedPools) / pools.length * 100).toFixed(1)
      }%)`,
    );
    ...
    if (failedPools > 0) {
      ...
      for (const [color, failures] of Object.entries(colorFailures)) {
        if (failures > 0) {
          console.log(
            `  ${colorNames[color]}: ${failures} pools (${
              (failures / pools.length * 100).toFixed(1)
            }%)`,
          );
        }
      }
